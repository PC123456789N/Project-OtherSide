...
export default function CombatPage() {
    ...
    const monsters = [

        {
            ...
            skills: [
                { id: "skill-fortitude", name: "Fortitude", value: 3, bonus: 5, lastResult: null },
                { id: "skill-luta", name: "Luta", value: 3, bonus: 6, lastResult: null },
                { id: "skill-iniciativa", name: "Iniciativa", value: 2, bonus: 3, lastResult: null },
                { id: "skill-pontaria", name: "Pontaria", value: 1, bonus: 0, lastResult: null },
                { id: "skill-vontade", name: "Vontade", value: 2, bonus: 2, lastResult: null },
            ],

            // Ataques agora são 100% digitados pelo usuário — nada é puxado
            // de attributes/skills. testBonus é somado ao d20 no botão
            // "Teste"; threatMargin define a partir de qual número no d20
            // (sozinho) é considerado crítico.
            attacks: [
                {
                    id: "attack-claws",
                    name: "Claws",
            ...
            // Resistências agora só têm nome + descrição livre.
            resistances: [
                {
                    id: "resistance-blood",
                    name: "Sangue",
            ...
            description:
                "A creature twisted by the Blood element, driven only by violence.",
        },
    ];
    ...
    useEffect(() => {
        ...
        setSelectedEntity((prev) => ({
            ...prev,
            name: matchedCombat.name || prev.name,
            image: matchedCombat.image || prev.image,
            type: matchedCombat.type || prev.type,
            element: matchedCombat.element || prev.element,
        }));
    ...
    function handleAttackTestResult(attackId, result) {
        updateEntity((prev) => ({
            ...
            attacks: prev.attacks.map((attack) =>
                attack.id === attackId
                    ? { ...attack, lastTestResult: result.total, lastCritical: result.isCritical }
                    : attack
            ),
        }));
    }
    ...
    function handleAttackDamageResult(attackId, result) {
        updateEntity((prev) => ({
            ...prev,
            attacks: prev.attacks.map((attack) =>
                attack.id === attackId ? { ...attack, lastDamageResult: result.total } : attack
            ),
        }));
    }
    ...
    function handleAttackCritDamageResult(attackId, result) {
        updateEntity((prev) => ({
            ...prev,
            attacks: prev.attacks.map((attack) =>
                attack.id === attackId ? { ...attack, lastCritDamageResult: result.total } : attack
            ),
        }));
    }
    ...
    function handleResistanceRemove(resistanceId) {
        updateEntity((prev) => ({
            ...prev,
            resistances: prev.resistances.filter((resistance) => resistance.id !== resistanceId),
        }));
    }
    ...
    function handleResistanceChange(resistanceId, field, value) {
        updateEntity((prev) => ({
            ...
            resistances: prev.resistances.map((resistance) =>
                resistance.id === resistanceId ? { ...resistance, [field]: value } : resistance
            ),
        }));
    }
    ...
    function handleVulnerabilityRemove(vulnerabilityId) {
        updateEntity((prev) => ({
            ...prev,
            vulnerabilities: prev.vulnerabilities.filter((item) => item.id !== vulnerabilityId),
        }));
    }
