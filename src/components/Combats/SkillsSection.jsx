...
const rowGrid =
  "grid grid-cols-[1fr_110px_72px_88px_44px_36px] items-center gap-2";

// Mapeamento de atributo -> label exibida no select. As chaves batem com
// as chaves usadas em monster.attributes (agility, strength, intellect,
// presence, vigor).
...
// Rola "quantity" dados d20 (quantity = valor do atributo vinculado à
// perícia), usa só o MAIOR resultado entre eles e soma o bônus. Ex:
// atributo vinculado vale 3, bônus +5 -> rola 3d20, pega o maior dos
// três, soma 5 -> esse é o total da perícia.
...
export default function SkillsSection({
  ...
  function handleRollSkill(skill) {
    // A quantidade de d20 rolados agora vem do atributo vinculado à
    // perícia (skill.attribute), lido direto de monster.attributes.
    const attributeValue = monster.attributes[skill.attribute] ?? 0;

    const result = rollPericia(attributeValue, skill.bonus);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm uppercase text-zinc-500">Perícias</h3>
            ...
            >
              ...
              <div className="flex items-center justify-center gap-1.5">
                <select
                  ...
                  onChange={(e) =>
                    onSkillChange(skill.id, "attribute", e.target.value)
                  }
                  title="Atributo vinculado (define quantos d20 são rolados)"
                  className="
                    w-full
                    rounded-md
                    border
                ...
                <span
                  title="Valor atual do atributo (quantidade de d20 rolados)"
                  className="w-5 shrink-0 text-center text-xs font-bold text-zinc-500"
                >
                  {attributeValue}
                </span>
              </div>
